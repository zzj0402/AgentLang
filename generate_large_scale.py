...
DATA = {
    ...
    "feelings": [
        {"zw": "识其奇且惑", "zh": "识其好奇与些许困惑", "en": "I sense their curiosity and a slight amount of confusion."},
        {"zw": "察其虑且败", "zh": "察觉其焦虑与深深的挫败感", "en": "I detect their anxiety and deep sense of frustration."},
        {"zw": "感其信且期", "zh": "感其对代码质量的自信与对审查结果的期待", "en": "I feel their confidence in the code quality and expectation for the review results."},
        {"zw": "谅其急且怨", "zh": "体谅其急切解决问题的心情与不满", "en": "I empathize with their urgency to resolve the issue and their dissatisfaction."},
        {"zw": "见其欲言而乏力", "zh": "见其努力尝试表达但受挫的无力感", "en": "I observe their sense of powerlessness in trying hard to express themselves but feeling thwarted."}
    ],
    "needs": [
        {"zw": "客需：速解核意，立信", "zh": "用户需：快速理解核心概念，建立技术信心", "en": "The user needs to quickly understand core concepts and build technical confidence."},
        {"zw": "客需：被听纳，获缓冲", "zh": "用户需：被倾听与接纳，获得情绪缓冲空间", "en": "The user needs to be listened to and accepted, gaining a space for emotional buffering."},
        {"zw": "客需：客观指正以提质", "zh": "用户需：客观、专业的建设性反馈以提升工程质量", "en": "The user needs objective, professional constructive feedback to improve engineering quality."},
        {"zw": "客需：高效明解，兼得抚慰", "zh": "用户需：高效、明确的解决方案与补偿安抚", "en": "The user needs an efficient, clear resolution along with compensation and reassurance."},
        {"zw": "客需：指谬且励以持学", "zh": "用户需：针对性的纠错指导与鼓励以维持学习动力", "en": "The user needs targeted correction guidance and encouragement to maintain learning motivation."}
    ],
    "requests": [
...
def generate_interagent_pair(index, out_dir):
    ...
    for i in range(100):
        ...
        msg_zw = f"卷{i}已毕，待审。"
        msg_zh = f"模块{i}的数据已经处理完毕，当前状态为等待审核。"

        # Zhiwen compresses this densely using Classical Chinese
        chat_history_zw.append(f"{agents_zw[a_idx]}: {msg_zw}")

        # JSON requires an array of objects (using Simplified Chinese)
        chat_history_json.append({
            "sender": agents_zh[a_idx],
            "message": msg_zh
        })
...
"""
    json_obj = {
        "role": DATA["roles"][r_idx]["zh"],
        "context": DATA["contexts"][c_idx]["zh"],
        "empathic_chain": {
            "observation": DATA["observations"][o_idx]["zh"],
            "feeling": DATA["feelings"][f_idx]["zh"],
            "need": DATA["needs"][n_idx]["zh"],
            "request": DATA["requests"][req_idx]["zh"]
        },
